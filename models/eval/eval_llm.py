...
class EvalLLM:
    ...
    def __init__(self, args, manager=None):
        ...
        traj_path = args.traj_file.replace("data/json_2.1.0/", f"{args.model}/").replace("/traj_data.json", "")
    ...
    def setup_scene(self, env: ThorEnv, traj_data, args, reward_type='sparse'):
        ...
        if not event.metadata.get('lastActionSuccess', True):
            error_msg = f"[setup_scene] Failed to initialize agent position. Trying to forceAction. This is the previous error msg: {event.metadata.get('errorMessage', '')}"
            self.log(error_msg)

            # Retry with forceAction to tolerate larger adjustments
            init_action['forceAction'] = True
            event = env.step(init_action)

            if not event.metadata.get('lastActionSuccess', True):
                error_msg = f"[setup_scene] Retry with forceAction also failed: {event.metadata.get('errorMessage', '')}"
